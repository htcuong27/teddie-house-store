...
type ColorType = 'normal' | 'blur' | 'success' | 'action' | 'danger' | 'alert' | 'invisible' | 'orange' | 'yellow';
...
const ButtonWithIcon = ({ children, onClick, color = 'normal' }: ButtonWithIconProps) => {
    const listColor: Record<ColorType, string> = {
        ...
        'orange': 'bg-gray-50 opacity-90 text-third-color hover:opacity-100 border-t-zinc-200 first:border-r-zinc-200',
        'yellow': 'bg-gray-50 opacity-90 text-fifth-color hover:opacity-100 border-t-zinc-200 first:border-r-zinc-200'
    };
    ...
    return (
        ...
        <button onClick={onClick} type="button" className={`${defaultStyleButton} ${listColor[color]} ${flexText} first:rounded-tl-none last:rounded-tr-none first:rounded-tr-none first:rounded-br-none last:rounded-tl-none last:rounded-bl-none `}>
            {children}
        </button>
    );
};
